...
export function validateVAPIDKeys(): boolean {
	try {
		...
		if (
			!urlSafeBase64Regex.test(vapidPublicKey) ||
			!urlSafeBase64Regex.test(vapidPrivateKey)
		) {
			return false;
		}
...
export async function sendNotifications(
	...
	const failedSubscriptions: Array<{
		subscription: PushSubscription;
		error: string;
	}> = [];
...
export function createNotificationPayload(
	...
	return {
		title: `메이플팅 업데이트: ${statusText}`,
		body: `${nickname} ${statusText}`,
		nickname,
		status,
		timestamp,
		data: {
